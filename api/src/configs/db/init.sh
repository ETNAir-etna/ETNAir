...
if [[ -z "$ETNAIR_PUBLIC_USER_PASSWORD" || -z "$ETNAIR_USER_PASSWORD" || -z "$ETNAIR_ADMIN_MANAGER_PASSWORD" || -z "$ETNAIR_SUPER_ADMIN_PASSWORD" ]]; then
    echo "Les variables d'environnement nécessaires ne sont pas définies. Abandon."
    exit 1
fi

echo "databse inicialisation start"
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOF
    ...
    BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'etnair_public_user') THEN
            CREATE ROLE "etnair_public_user" LOGIN PASSWORD '${ETNAIR_PUBLIC_USER_PASSWORD}' INHERIT;
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'etnair_user') THEN
            CREATE ROLE "etnair_user" LOGIN PASSWORD '${ETNAIR_USER_PASSWORD}' INHERIT;
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'etnair_admin_data_manager') THEN
            CREATE ROLE "etnair_admin_data_manager" LOGIN PASSWORD '${ETNAIR_ADMIN_MANAGER_PASSWORD}';
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'etnair_super_admin') THEN
            CREATE ROLE "etnair_super_admin" LOGIN PASSWORD '${ETNAIR_SUPER_ADMIN_PASSWORD}' SUPERUSER;
        END IF;

        GRANT "etnair_admin_data_manager" TO "etnair_super_admin";

    END \$\$;
EOF

echo "The database was successfully initialized"